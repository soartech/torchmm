...
class CategoricalModel(Model):
    ...
    the HMM. Uses a categorical distribution and a Dirchlet prior. The model
    emits integers from 0-n, where n is the length of the probability tensor
    used to initialize the model.
    """

    def __init__(self, n_emissions: int,
                 probs: Optional[torch.Tensor],
                 prior: Optional[Union[float, int, torch.Tensor]] = None,
                 device: str = "cpu") -> None:
        """
        Creates a categorical model that emits values from 0 to the provided
        n_emissions. This model defaults to uniform probability of emitting
        each value; however, an optional prob vector allows the emission
        probabilities to be provided. A Dirchlet prior, or emission psuedo
        counts, can also be provided, which is used primarily during model
        fitting.

        :param n_emissions: the number of discrete emissions generated by the
            model. The model outputs integers from 0 to n_emissions.
        :param probs: a tensor of probablities for each discrete emission, must
            sum to 1.
